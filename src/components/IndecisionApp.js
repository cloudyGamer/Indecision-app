...
/////////////////////////////////////////////////////////// main component
   ...
   this.masterObj = {
     testVal: "testVal",
       ...
       removeBasketHandler : this.removeBasketHandler,
       getCartId : this.getCartId,
       APIRemoveItem : this.APIRemoveItem,
       fncAddToggle : this.fncAddToggle,
       delayBasket : this.delayBasket,
       removeItemWrapper: this.removeItemWrapper,
       fncRemoveToggle : this.fncRemoveToggle,
       fetchCategory : this.fetchCategory,
       delaySection: this.delaySection,
       pageOpen: this.pageOpen,
       loopState: this.loopState,
       delayBasketRemove : this.delayBasketRemove,
       extractDescription: this.extractDescription,
   ...
   testVal: "testVal",
     category: "Bath Bombs",
     api_token: undefined,
     fetchUrl: "http://localhost:8888/opencart-3.0.3.1/upload/getCatagory.php",
     package: {address: "http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=product/category/apiCategory",
       searchQuery: {}},
   ...
   testVal: "basketObj: testValueBilly",
     category: "Bath Bombs",
     fetchUrl: "http://localhost:8888/opencart-3.0.3.1/upload/APINoBody.php",
     address: "http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=api/cart/products&api_token=",
   ...
   testVal: "testVal",
     ...
     payLoad: {
      fetchUrl: "http://localhost:8888/opencart-3.0.3.1/upload/getCatagory.php",
      package: {address: "http://localhost:8888/opencart-3.0.3.1/upload//index.php?route=api/cart/remove&api_token=",
         searchQuery: {}}},
...
//<editor-fold defaultstate="collapsed" desc="fnc and cdm"> 
   setHomePage = (uniqueMessage) => {
     console.log("setHomePage executed");
     //console.log(uniqueMessage + "set component obj+ " + JSON.stringify(this.homePageObj));
     this.setState(
     {
     currentComponent: HomePage,
       currentObj: this.homePageObj
     }, function () {
     console.log(uniqueMessage + "state.currentObj" + this.state.currentObj)
     });
}
////////////////////////////////////////
      setStoreInfo = (uniqueMessage) => {
     console.log("setStoreInfo executed");
     console.log(uniqueMessage + "set component obj+ " + JSON.stringify(this.homePageObj));
     this.setState(
     {
     currentComponent: StoreInfo,
       currentObj: this.homePageObj
     }, function () {
     console.log(uniqueMessage + "state.currentObj" + this.state.currentObj)
     });
...
//////////////////////////////////////// basketHandler
   basketHandler = async (e = undefined, product_id = undefined, quantity = undefined) => {
   ...
   //this.toggleState("basketClicked");
//   ////////////////////////////////////////
     if (this.state.tokenRequested === false) {
          
    this.apiRequest2().then(promise =>
      this.APIAddToCart(promise, product_id, quantity))
      .then(promise => this.APIGenericNoBody(this.basketObj,"basktHndler:","basketState",this.state.promise.api_token));
...
//////////////////////////////////////// APIAddToCart
   APIAddToCart = async (api_token, product_id, quantity) => {
...
//     console.log("APIaddtocart - token equals___" + api_token);
     const url = "http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=api/cart/add&api_token=" + api_token;
     const method = "POST";
...
//      }*/
//
     return fetch("http://localhost:8888/opencart-3.0.3.1/upload/addToCart.php", {method, body})
     .then(response => response.json())
     .then(response => JSON.parse(response))
     .then(data =>
       this.setState({addToCartResponse: data})
       )
     .then(promise =>
      console.log("anymessage"+JSON.stringify(promise))
       );
   }
////////////////////////////////////////
   apiLogIn = (api_token) => {
   console.log(" apiLogIn fired");
     const url = "http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=api/shipping/address&api_token=" + api_token;
     const method = "POST";
     const body = new FormData();
     body.append("url", url);
     fetch("http://localhost:8888/opencart-3.0.3.1/upload/logInAPI.php", {method, body})
     .then(res => res.json())
     .then(response => JSON.parse(response))
     .then(promise =>
...
extractDescription = (description) => {
     ...
     const ingredientsArray = ingredientArray0[0];
      ...
      //return {__html: ingredientsArray};
...
////////////////////////////////////////
   ...
   console.log("fetchCategory executed with parameter" + data + "from within" + uniqueMessage);
       return window.fetch(`http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=product/product/getProductCategory&product_id=` + data, {
     method: 'GET', // or 'PUT' 
       headers: {}
     }).then(res => res.json());
 ...
 ////////////////////////////////////////
   fncRemoveToggle = (fnc,key,product_id,localState,quantity) => {
          
          const uniqueMessage = "fncToggle";
          localState(key,uniqueMessage);
          console.log(uniqueMessage+"state set");
          let e = undefined;
          fnc(product_id);

          console.log("fncToggle executed");    
     }
 ////////////////////////////////////////    
   fncAddToggle = (fnc,key,product_id,localState,quantity) => {
          const uniqueMessage = "fncToggle";
          localState(key,uniqueMessage);
          console.log(uniqueMessage+"state set");
          let e = undefined;
          fnc(e,product_id,quantity);
          //this.state1.product_id
          console.log("fncToggle executed");
     }
////////////////////////////////////////
   delayBasket = (e,product_id,basket,fncToggle,localState,key,quantity) => {
      if(e!=="featProd"){
           const ed = document.getElementById("ed");
           ed.className = "edUnclicked openAnimationEdUnclicked";
           const basketUnclicked = document.getElementById("basketUnclicked");
           basketUnclicked.className = "midshelf_addToCart basketUnclicked openAnimationBasketUnclicked";
      }
      ...
      setTimeout(
          function(){
               fncToggle(addBasket,key,product_id,localState,quantity);
          },800
          ); 
        //this.fncToggle(addBasket,"basketClicked");
     }
////////////////////////////////////////
   delayBasketRemove = (e,product_id,basket,fncToggle,localState,key,quantity) => {
      if(e!=="featProd"){
           const ed = document.getElementById("basketClicked");
      ed.className = "midshelf_addToCart basketClicked closeAnimationBasketClicked";
      }
...
////////////////////////////////////////
   ...
   delaySection = (e,localState,key,id,className,arrowClass) => {
      console.log("delaySection executed");
      let uni = "delaySection: ";
     let el = document.getElementById(id);
          console.log("delaySection className= "+el.className);
          el.className = className;
          const target = document.getElementById(e);
          console.log("target "+target.className);
         let titleHolder= target.firstElementChild;
          console.log("firstelement"+titleHolder.className);
        let arrow = titleHolder.nextElementSibling;
         console.log("secondelement"+arrow.className);
...
////////////////////////////////////////
 ////////////////////////////////////////    
   sectionToggle = (fnc,key,localState) => {
          localState(key);
          const uniqueMessage = "fncToggle";
          console.log(uniqueMessage+"state set");
          let e = undefined;
...
////////////////////////////////////////
   removeItemWrapper = (product_id) => {
         const cartId = this.getCartId(product_id);
         console.log("returned promise"+cartId);
         this.removeBasketHandler("fncToggle",cartId);
     }
////////////////////////////////////////
   getCartId = (product_id) => {
    const uniqueMessage = "getCartId";
     console.log("getCartId executed looking for product_id "+product_id);
     const stateToSearch = this.state.basketState.products;
     const cartIdArray = stateToSearch.map(function (key) {
          if(key.product_id==product_id){
             return key.cart_id
          }else{
               return "no match found"}
          console.log ("getCartId "+JSON.stringify(key.cart_id)+" product_id "+key.product_id);

     });
     console.log(uniqueMessage+" cart_id="+JSON.stringify(cartIdArray[0]));
     const cartId = cartIdArray[0];
     return cartId
     
}
////////////////////////////////////////
   toggleState = (key) => {
        console.log("toggleState executed this.state."+key+"key= "+this.state[key]);
        this.setState(prevState => ({
        [key]: !prevState[key]
}));
   }
////////////////////////////////////////
   removeBasketHandler = (uniqueMessage,cart_id) => {
     uniqueMessage = uniqueMessage + "RMVBASKET:"
     console.log(uniqueMessage+cart_id);
     const removeBasketItem = this.APIRemoveItem;
     const updateBasket = this.APIGenericNoBody;
     
     //this.toggleState("basketClicked");
     return  removeBasketItem(this.state.promise,"key",cart_id,this.removeBasketItem,uniqueMessage)
     .then(promise => updateBasket(this.basketObj,"rmvBasktHndler:","basketState",this.state.promise.api_token));
...
////////////////////////////////////////
   ...
   console.log(uniqueMessage + " APIGenericNoBody call ran");
     ...
     if (obj !== undefined){
          console.log(uniqueMessage+"obj you want"+JSON.stringify(obj));
          console.log(uniqueMessage+"obj keys you want"+Object.keys(obj));
...
//////////////////////////////////////// APICategoryPostSave
   ...
   console.log(uniqueMessage+ "obj parameter keys"+Object.keys(obj));
...
//.then(data => console.log(JSON.stringify(obj), null, "\t"))
   ...
   ////////////////////////////////////////
   APIRemoveItem = (promise, propertyKey, path, obj, uniqueMessage) => {
   console.log(uniqueMessage + "APIRemoveItem call ran");
   console.log(uniqueMessage + "obj parameter" + JSON.stringify(obj));
   console.log(uniqueMessage+ "obj parameter keys"+Object.keys(obj));
     //console.log(uniqueMessage+"propertyKey test"+Object.keys(obj.payLoad.package.searchQuery));
     //console.log(uniqueMessage + "category object path Keith" + JSON.stringify(obj.payLoad.package.searchQuery));
...
//
     ...
     return data
       ...
       .then(data => console.log(uniqueMessage+"promised data"+JSON.stringify(data)));
...
//console.log("this.state.promise equals___"+this.state.promise);
     return  fetch("http://localhost:8888/opencart-3.0.3.1/upload/requestToken.php",
     {method, body})
     .then(response => response.json())
     .then(response => JSON.parse(response))
     .then(data =>
...
//             }
//           );
     return  fetch("http://localhost:8888/opencart-3.0.3.1/upload/requestToken2.php",
     {method, body})
     .then(response => response.json())
     .then(response => JSON.parse(response))
     .then(data =>
...
////////////////////////////////////////
   /*APIAddCustomer = (e,form) => {
    ...
    //return window.fetch("http://localhost:8888/opencart-3.0.3.1/upload/addNewCustomer.php&firstname="+firstname+"&lastname="+lastname+"&email="+email+"&telephone="+telephone 
    return window.fetch("http://localhost:8888/opencart-3.0.3.1/upload/addNewCustomer.php"                
...
/////////// apiRequest
   ...
   console.log(" apiRequest fired");
     return window.fetch("http://localhost:8888/opencart-3.0.3.1/upload/requestToken.php"
       , {
...
//console.log("fetch executed" );
   console.log("fetch executed with parameter" + data + "from within" + uniqueMessage);
     ...
     return  window.fetch(`http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=` + data, {
     method: 'GET', // or 'PUT' 
...
//////////////////////////////////////// menu click handler
   ...
   console.log("Indec:menuHandleClick called");
     ...
     return  this.noCurlCall("http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=product/category/apiCategories", "Indec:CDM", this.categoryObj);
   }
//////////////////////////////////////// APIGenericPostSave
...
//</editor-fold>
   ...
   console.log("loop state" + JSON.stringify(uniqueMessage + "passedState category_id" + JSON.stringify(state[key].category_id) + "category"));
     ...
     categoryObj.category = state[key];
...
//<editor-fold defaultstate="collapsed" desc="lets">
     ...
     let currentObj;
     let slideMenuScroll = "";
     //let availableCategories = this.noCurlCall("http://localhost:8888/opencart-3.0.3.1/upload/index.php?route=product/category/apiCategories","Indec:CDM",this.categoryObj);
...
//<editor-fold defaultstate="collapsed" desc="comment">
   ...
   if(this.state.toolBarClass){
        slideMenuScroll = "slideMenuScroll";
   }else{
        slideMenuScroll = "slideMenuUnscroll";
   }
   ...
   slideMenu = slideMenuScroll+" "+slideMenu;
...
////////////////////////////*eturn    
   return (
<div id="mainApp" className="appDiv scrollTop" ref={this.scrollRef} onScroll={this.handleScroll} xmlns="http://www.w3.org/1999/xhtml" >
          ...
          <div className={this.state.toolBarClass ? 'toolBarWrapperScroll':'toolBarWrapper'}  >
               ...
               <div className="hamburgerMenu">
                         ...
                         <HamburgerMenu
                             ...
                             width={18 * this.menuMult}
                             height={15 * this.menuMult}
                             strokeWidth={1}
                             rotate={0}
                             color='White'
                             borderRadius={0}
          ...
          <div className={slideMenu}>
                    ...
                    this.loopState(this.state.noPromise, this.state.categories, "Indec:ren: loopState", this.categoryHandlerObj, this.APICategoryPostSaveGlobal)
                    }</div>
          </div>  
